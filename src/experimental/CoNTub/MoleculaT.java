...
class MoleculaT extends MoleculaB {
	...
	//ESTO NOS AUTORIZA PARA ELIMINAR EL USO DE MAUX
	pto3D proyecta(pto3D psus)
        {
		MoleculaT molau=new MoleculaT();
	...
	pto3D proyecta(pto3D sustituido,MoleculaT maux) {
		...
		/*    	double dh=pc.dihedro(TESTC,pp);
			...
			pc=pc.escala(-1); pc.versoriza();
			}
			else {
			pc.x=-10;pc.y=0;pc.z=0;addVert(pc,1);
			}//MARCA EL ERROR con un punto -10,0,0 ROJO
		*/
		pc=pc.escala(-1);pc.versoriza();//asociado a esto de arriba!
		...
		if (sustituido.x<=TEST1.x) sustitucion=sust1.clona();
		else {
			if (std==true) {
				...
				if (pro>TESTC.modulo()) sustitucion=sust2.clona();
	...
	void centraentorno(int num) {
		...
		if (numvert==3) {
			for (int i=0;i<nvert();i++) {
				pto3D ptov=vert(i);
				if (ptov.dista(pto)<1.6) posnueva=posnueva.mas(ptov.escala(0.333));
			}
			mueve(num,posnueva);
		}
	}
	...
	Mensaje sniff(pto3D candidato, int gen) {
		...
		for(int i=0;i<nvert();i++) {
			...
			if (dista<0.5) ocupado=true; else if (dista<2.0 && angulo>90) {
				nvecnorte++; nvn[nvecnorte]=i;
			}
			if (dista<0.5) ocupado=true; else if (dista<2.0 && angulo<90) {
				nvecsur++  ; nvs[nvecsur  ]=i;
			}
			if (dista<0.5) ocupado=true; else if (dista<2.0 && angulo<165 &&angulo>100) {
				nveccono++;nvc[nveccono]=i;
			}
			if (dista<5.0) nvecpond=nvecpond+Math.exp(-Math.pow((dista-1.5)/0.5,2));
		}

		//Y AQUI decidimos, en funcion de esos numeros, si tiene 0, 1, 2  o mas (erroneo) vecinos (descontando el
		// si hay coincidencia
		if  (ocupado) {
			cadena="\nPossition Occupied";nvecinos=-1;
		}
		else if(nveccono==nvecnorte && nveccono==(int)nvecpond) nvecinos=nveccono;
		else if(nveccono==nvecnorte)     {
			cadena=""+nvecpond; nvecinos=nveccono;
		}
		else if(nveccono!=nvecnorte)     {
			nvecinos=9;cadena="\nWarning: muy complicado!! "+nveccono+" "+nvecnorte;
		}
		else   {
			cadena="\nSituacion no considerada:"+nveccono+" "+nvecnorte;  nvecinos=9;
		}
		...
		if      (nvecinos==0) {
			//por ahora lo añadimos y ya esta.
			ptoprop=candidato.clona();recom=1;
		}
		else if (nvecinos==-1) {
			...
			//medimos distancia al vecino y al generador, y el pto
			...
			//por ahora se calcula la media de los tres
	...
	String sniff2(int gen) {
		...
		//ERROR, el punto de generacion esta solo!!
		if      (nv==0) {
	                cadena=cadena+"\nERROR: no neighbors around atom "+gen;

		}
		else if (nv==1) {
			//Usar ideas del viejo sniff Por ahora, solo añadimos
			...
			anillo anillo1=new anillo();pto3D vv1=new pto3D();
			anillo anillo2=new anillo();pto3D vv2=new pto3D();
			...
			if     (anillo1.num==2) {
				//anillo vacio, fijamos el vectorcillo propuesto
				vv1=cand.menos(vert(gen));
			}
			else if(anillo1.num==3) {
				//anillo desperdigado, o vacio fijamos al candidato
				vv1=cand.menos(vert(gen));
			}
			else {
				//Comprobamos que no hay huecos entre los tres primeros pasos del anillo o proyecto de anillo
				/*	OJOOO*/	          double da1=vert(anillo1.vert[1]).dista(vert(anillo1.vert[2]));
				...
				if (da1<2.0 && db1<2.0 && dc1<2.0) {
					...
					//Hay algun hueco, ante la duda, proponer el de antes
					vv1=cand.menos(vert(gen));
				}
			}
	...
	/*
	  ...
	  double ddm=0,ddav=0;
	  for (int i=1;i<=6;i++) {
	  pto3D pa=anref[i].menos(anori.centroide);
	  ...
	  for (int i=1;i<=6;i++) {
	  pto3D pref=anref[i].menos(this.vert[anori.vert[i]]);
	  ...
	  //Tiene que devolver un anillo!
	  anillo findring(pto3D ptoref, int ref)
	  {
	  ...
	  if (dista<distan && ang>90 && (Math.abs(dh)<65)) {
	  distan=dista;sigtemp =i;
	  }
	  }
	  }
	  ...
	  if   (this.ocupa(p1c,0.8)) {
	  p1c=this.vert[atomoqueocupa(p1c,0.8)].clona();conecta(a1,atomoqueocupa(p1c,0.8));a1c=atomoqueocupa(p1c,0.8);
	  }
	  else                       {
	  addVert(p1c,6);                              ;conecta(a1,nvert);                 a1c=nvert;
	  }
	  //el segundo es el anterior mas el salto del opuesto
	  pto3D p1l=p1c.mas(ava2);
	  p1l=proyecta(p1l);//proyectado
	  if   (this.ocupa(p1l,0.8)) {
	  p1l=this.vert[atomoqueocupa(p1l,0.8)].clona();conecta(a1c,atomoqueocupa(p1l,0.8));a1l=atomoqueocupa(p1l,0.8);
	  }
	  else                       {
	  addVert(p1l,6);                              ;conecta(a1c,nvert);                 a1l=nvert;
	  }

	  //Brazo 2
	  pto3D p2c=cideprev.mas(ava2.escala(2)); //pto cercano
	  p2c=proyecta(p2c);//proyectado
	  if   (this.ocupa(p2c,0.8)) {
	  p2c=this.vert[atomoqueocupa(p2c,0.8)].clona();conecta(a2,atomoqueocupa(p2c,0.8));a2c=atomoqueocupa(p2c,0.8);
	  }
	  else                     {
	  addVert(p2c,6);                                ;conecta(a2,nvert);                 a2c=nvert;
	  }
	  //el segundo es el anterior mas el salto del opuesto
	  pto3D p2l=p2c.mas(ava1);
	  p2l=proyecta(p2l);//proyectado
	  if   (this.ocupa(p2l,0.8)) {
	  p2l=this.vert[atomoqueocupa(p2l,0.8)].clona();conecta(a2c,atomoqueocupa(p2l,0.8));a2l=atomoqueocupa(p2l,0.8);
	  }
	  else                       {
	  addVert(p2l,6);                               conecta(a2c,nvert);                 a2l=nvert;
	  }
	  conecta(a1l,a2l);
	  ...
	  String m1,m2;
	  if (ocupa1(cd1d))    {
	  con1=atomoqueocupa1(cd1d);conecta(con1,con);m1=exploraanillo(con1);
	  }
	  else {
	  addVert(cd1d,7);con1=nvert;               conecta(con1,con);m1=exploraanillo(con1);
	  }



	  if (ocupa1(cd2d))    {
	  con2=atomoqueocupa1(cd2d);conecta(con2,con);m2=exploraanillo(con2);
	  }
	  else {
	  addVert(cd2d,8);con2=nvert;               conecta(con2,con);m2=exploraanillo(con2);
	  }
	  ...
	  return "ERROR: unknown connectivity"+mconec[con][0];
	  }
	...
	*/
	...
	void cierraN() {
		for (int i=0;i<nvert();i++) {
			...
			if      (mc[0]==1) {
				...
				for(;sale=false;) {
					...
					if     (mctemp[0]==1)	{
						marcaborra(conectado);sale=true;
					}
					//se acabo, el hilo se corta, se marca como borrable, y se sale
					else if(mctemp[0]==2) {
						//de todos modos marcamos para borrar, v
						marcaborra(conectado);
						if      (mctemp[1]!=atprev) {
							atprev=conectado;conectado=mctemp[1];
						}//la pos 1 es la siguien
						else if (mctemp[2]!=atprev) {
							atprev=conectado;conectado=mctemp[2];
		...
		for (int i=0;i<nvert();i++) {
			//HAcemos un segundo pase para nitrar los flecos cerrados sobre si mismos
			...
			if (mc[0]==2) {
				// Sustitucion N y no hace falta ver conectividades de nuevo, porque ya esta bien, y lo que hariamos
	...
	//METODO A usar solo con estructuras grafiticas
	int remataconec() {
		int nr=0;
	...
	//Metodo alternatiov, que revisa todos los atomos con
	void remataconec2() {
	...
	String exploraanillo(int atocentro) {
		...
		if (mc[0]==1) {
			...
			if (mc[0]==2) {
				...
				an.addVert(atobif);
				if (mc[1]!=atocentro) an.addVert(mc[1]);
				else                  an.addVert(mc[2]);

				//ya tenemos tres, sigamos con el algoritmo estandar
				boolean as=true; //Por defecto pensamos que el anillo sigue
				//as es anillosigue: chequea si el anillo tiene continuidad
				for (int i=3;as;i++) {
					boolean admitido=false;
				int mci[]=((Atomo)susatomos.get(an.vert[i])).mconec;
				//Buscamos entre todos los conectados
				for (int k=1;k<=mci[0];k++) {	
					int cdto=mci[k];       //este es el candidato
				double dhd=dihedro(cdto,an.vert[i],an.vert[i-1],an.vert[i-2]);
				//que forma un diedro con los veteranos de ana
				if ((cdto!=an.vert[i-1])&&(Math.abs(dhd)<90.0)) {
					an.addVert(cdto);admitido=true;
				}
			...
			if (mcbif[0]!=3) return "Atomo bifurcado presenta mala conectividad ("+mcbif[0]+")";
			...
			for (int i=1;i<=3;i++) {
				...
				if (cd4==atocentro ) {}
					...
					j++;
					if      (j==1) ana.addVert(cd4);
					else if (j==2) anb.addVert(cd4);
					else           return "Error indeterminado: anillos A y B tienen "+ana.num+" y "+anb.num+" miembroe: Atobif es "+atobif;
				}
			}
			...
			for (int i=3;asa;i++) {
				...
				for (int k=1;k<=mci[0];k++) {
					int cdto=mci[k];       //este es el candidato
					double dhd=dihedro(cdto,ana.vert[i],ana.vert[i-1],ana.vert[i-2]);
					//que forma un diedro con los veteranos de ana
					if ((cdto!=ana.vert[i-1])&&(Math.abs(dhd)<90.0)) {
						ana.addVert(cdto);admitido=true;
					}
					//Y lo sumamos a ana si no vamos a atras y es razonablemente plano...
				}
			...
			for (int i=3;asb;i++) {
				...
				for (int k=1;k<=mci[0];k++) {
					int cdto=mci[k];       //este es el candidato
					double dhd=dihedro(cdto,anb.vert[i],anb.vert[i-1],anb.vert[i-2]);
					//que forma un diedro con los veteranos de ana
					if ((cdto!=anb.vert[i-1])&&(Math.abs(dhd)<90.0)) {
						anb.addVert(cdto);admitido=true;
					}
					//Y lo sumamos a ana si no vamos a atras y es razonablemente plano...
				}
				//Una vez recorridos todos los candidatos,chequeamos las posibilidades que haran que as sea falso
				//Que ninguno haya sido admitido
				if (admitido==false) asb=false;
				//que el admitido	 no tenga vecinos (pero eso es imopsible, ya tiene como minimo 1
			}
			...
			if ((ana.num==6)&&(anb.num==6)) {
				//Si ambos son 6, lo ponemos en el centro de los tres posiciones circundantes.
			...
			return "Caso 6,X: desplazamiento lateral guiado por atomo "+anb.vert[5]+" D enlace= "+bond;
				...
				return "Caso 5,5: desplazamiento lateral guiado por atomos "+anb.vert[5]+" y "+ana.vert[5]+" D enlace= "+(bond1/2+bond2/2);
			}
				...
				if (dbond>1.8) dbond =dbond- (dbond-1.8)/2;
				if (dbond>1.2) dbond =dbond- (dbond-1.2)/2;
				vr.versoriza();
			...
			else if ((ana.num==5)&&(anb.num<=4)) {
				//hacemos que sea paralelo al brazo 5
				pto3D v=vert(ana.vert[5]).menos(vert(ana.vert[4]));
				v.versoriza();
				double bond=dmedia(ana);
				pto3D ve=v.escala(bond);//OJO A ESTOS PARAMETROS; DEBERIAN SER FLEXIBLES
				...
				//atenuamos las longitudes de enlace un poco
				if (dbond>1.8) dbond =dbond- (dbond-1.8)/2;
				if (dbond<1.2) dbond =dbond- (dbond-1.2)/2;   //OJO ESTO PUED SE IMPORTANTE? EN VE Z DE < habia >
			...
			else     return "Caso no recogido"+ana.num+","+anb.num+": Sin conexiones ni desplazamiento";
			...
			for (int i=1;i<=mca2[0];i++) {
				if ((mca2[i]!=ana.vert[1]) &&
				    (Math.abs(dihedro(anb.vert[2],anb.vert[1],ana.vert[2],mca2[i]))>90)) {
					asa=true;
					ana.addVert(mca2[i]);
				}
			}
			...
			for (int i=1;i<=mcb2[0];i++) {
				if ((mcb2[i]!=anb.vert[1])               &&
				    (Math.abs(dihedro(ana.vert[2],ana.vert[1],anb.vert[2],mcb2[i]))>90)) {
					asb=true;
					anb.addVert(mcb2[i]);
				}
			}
			...
			if ((ana.num==6)&&(anb.num==6)) {
				if   (ana.vert[6]==anb.vert[6]) {
					conecta(ana.vert[1],ana.vert[6]);return "new connection between "+atocentro+" and "+ana.vert[6];
				}
				else return "excessive connections near atom "+atocentro;
			}
			else {
				if (ana.num==6) {
					conecta(ana.vert[1],ana.vert[6]);return "new connection between "+atocentro+" and "+ana.vert[6];
					...
					conecta(anb.vert[1],anb.vert[6]);return "new connection between "+atocentro+" and "+anb.vert[6];
				}
	...
	double dmedia(anillo ani) {
		double dm=0;
		int n=ani.num-2;
		for (int i=2;i<ani.num;i++) dm=dm+distancia(ani.vert[i],ani.vert[i+1])/n;
		return dm;
	}
}
