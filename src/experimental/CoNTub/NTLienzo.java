...
public class NTLienzo extends JPanel { //Creamos la clase NTLienzo, que es una subclase de Canvas
...
public void NTLienzo(){NT=new Nanotubo(5,5);}

public void paint(Graphics g) {//Primer metodo, para dibujar este objeto. implementa graficos a traves del objeto g
	int LA=getWidth();
	int LL=getHeight();
	int bx0=25;
	int by0=130;
	...
	double nde[]=new double[121];//Calculo de DENSIDAD DE ESTADOS
	   ...
	   double mommk=momm*2*d/NT.i1*(Math.sin(Math.PI/3-NT.quiral()));
		...
		if(momz*momz+mommk*mommk<3.2){//           momy<2*d+momx*Math.sqrt(3)){
	       ...
	       if (Math.abs(e1-e2)>0.002){nde[orden]=nde[orden]+deltamz/Math.abs(e1-e2);} //
   ...
   for (int i=-3;i<=3;i++)g.drawLine(bx0,by0+(int)byl*i,bx0-5,by0+(int)byl*i);
   ...
   for (int ii=1;ii<=3*byl;ii++){
   		if (nde[ii]>1) g.drawLine(dx0+ii,dy0-2,dx0+ii,dy0-2-(int)(nde[ii]/2)); //NDESTADOS
   		if (nde[ii]>1) g.drawLine(dx0-ii,dy0-2,dx0-ii,dy0-2-(int)(nde[ii]/2));
   		//g.drawString(" "+nde[ii], 400,10*ii);
   		}

   g.setColor(Color.red);
   g.drawLine(dx0-3*byl,dy0,dx0+3*byl,dy0);
   for (int i=-3;i<=3;i++)g.drawLine(dx0+(int)byl*i,dy0,dx0+(int)byl*i,dy0-2);
   g.setColor(Color.blue);
   ...
   g.drawString("Density of States",dx0-3*byl-5,dy0+28);
    ...
    public void redraw(int n, int m) {  //Segundo metodo, de redibujado, parametrizado
	NT=new Nanotubo(n,m);
	repaint();  }
}
