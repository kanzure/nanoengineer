...
public class Mira3D extends JPanel implements MouseListener,MouseMotionListener,KeyListener{
	...
	public void paint(Graphics g){
		...
		if (!lrd) //solo con display normal
		{
			...
			for (int i=1;i<=nplanos;i++) morden[i][0]=0;
			//calculamos la matriz de orden
			for (int i=0;i<mmolg.nvert;i++)
			{
				int indice=(int)(2*(dim+mmolg.miniverts[i].z));
				//multiplicamos por 2, para duplicar el numero de rodajas por angstrom.
				morden[indice][0]++;
		...
		if (!lrd) {
			for (int i=planoini;i>=1; i--){
				...
				for (int j=1;j<=morden[i][0];j++){
					...
					if (autocon) g2.setColor(mmolg.minicolor[ii]);else  g2.setColor(Color.black);

					for (int k=1;k<=nconect;k++)
					{pto3D w=null;
					if   (autocon)  w=mmolg.miniverts[mmolg.miniconec[ii][k]].clona();
					else	   		w=mmolg.miniverts[mmolg.miniconec[ii][k]].clona(); //Conectividad alternativa
					...
					if (bst) g2.fillOval(biasx+x-r,biasy+y-r,2*r,2*r);
					else {
						if (atn) {
							g2.setColor(Color.white);g2.fillOval(biasx+x-r,biasy+y-r,2*r,2*r);
						} else if (nconect==0) {
					...
					if (atn)  {
						if (bst) g2.setColor(Color.white);
		...
		else {for (int i=0;i<mmol.nvert; i++){
		...
		if (nselec==1) {
			...
			pto3D v=mmol.miniverts[selec[1]];pto3D w=mmol.miniverts[selec[2]];
			g2.drawString(""+selec[1]+"-"+selec[2]+" distance = "+forma.aCadena(v.dista(w)),10,getHeight()-10);
		} else if (nselec==3) {
			pto3D v=mmol.miniverts[selec[1]];pto3D w=mmol.miniverts[selec[2]];pto3D u=mmol.miniverts[selec[3]];
			g2.drawString(""+selec[1]+"-"+selec[2]+"-"+selec[3]+" angle = "+forma.aCadena(u.menos(w).angulocong(v.menos(w))),10,getHeight()-10);
		} else if (nselec==4){
			pto3D v=mmol.miniverts[selec[1]];pto3D w=mmol.miniverts[selec[2]];pto3D u=mmol.miniverts[selec[3]];pto3D s=mmol.miniverts[selec[4]];
			pto3D a=v.menos(w);pto3D b=w.menos(u);pto3D c=s.menos(u);
			g2.drawString(""+selec[1]+"-"+selec[2]+"-"+selec[3]+"-"+selec[4]+" dihedral = "+forma.aCadena(a.dihedrog(b,c)),10,getHeight()-10);
		} else { }
	...
	public synchronized void setPainted(){pintado=true; notifyAll();}

	public void mousePressed (MouseEvent ev){Ox=ev.getX();Oy=ev.getY();}
	...
	public void mouseClicked (MouseEvent ev){
		...
		if (nselec==5){deselec();}
			...
			for (int i=0;i<mmol.nvert;i++){
				...
				if (dist<dista) {dista=dist;prox=i;  }
			}
			this.selec[nselec]=prox;

			if      (ev.getButton()==1) mmol.selecciona(prox,1);
			else if (ev.getButton()==3) deselec();
	...
	public void mouseReleased(MouseEvent ev) {
		if (!lrdf) lrd=false; else lrd=true;
		repaint();
	}
	public void mouseEntered (MouseEvent ev){requestFocus();}
	public void mouseExited  (MouseEvent ev){}
	public void keyTyped(KeyEvent ev){}

	public void keyPressed(KeyEvent ev) {
		int kc=ev.getKeyCode();
		if      (kc==32) {resetea();this.repaint();}//Espacio
		else if (kc==33) {if (escala<200) escala*=1.2;;this.repaint();}//Re Zoom +10%
		else if (kc==34) {if (escala>2)   escala*=0.8;;this.repaint();}//Av Zoom -10%
		else if (kc==38) {biasy-=20;this.repaint();}//ARR
		else if (kc==39) {biasx+=20;this.repaint();}
		else if (kc==40) {biasy+=20;this.repaint();}
		else if (kc==72) {if (this.hlp==true) this.hlp=false;else this.hlp=true;this.repaint();} //H para ayuda
		else if (kc==66) {if (this.bst==true) this.bst=false;else this.bst=true;this.repaint();} //B para Ball
		else if (kc==76) {if (this.atn==true) this.atn=false;else this.atn=true;this.repaint();} //L para labels
		else if (kc==67) {if (this.hbk==true) this.hbk=false;else this.hbk=true;this.repaint();} //C para corte
		else if (kc==65) {if (this.abt==true) this.abt=false;else this.abt=true;this.repaint();} //A para about
		else if (kc==82) {if (this.lrd==true) { this.lrd=false;this.repaint();this.lrd=true;} }// R para Render temporal
		else if (kc==37) {biasx-=20;this.repaint();}
		else if (kc==37) {biasx-=20;this.repaint();}
		else if (kc==37) {biasx-=20;this.repaint();}
		else if (kc==10) {this.repaint();} //enter
		else             {}
	...
	public void resetea() {
		//Molecula
		mmolg=mmol.clona();
		if (mmol.nvert>2000) lrdf=true; else lrdf=false;
		//Seleccion
		selec=new int[5];
		nselec=0;
		this.deselec();
	...
	public void mouseDragged (MouseEvent ev) {
		...
		{pintado=false;
		Nx=ev.getX();Ny=ev.getY();               //Cojo las nuevas posiciones cartesianas
		...
		if (mmol.nvert>800) lrd=true;
		else lrd=false;
		repaint();
		Ox=Nx;Oy=Ny;                  //una vez usadas, ya son las antiguas
		setPainted();

		}
	}
	void deselec(){
		this.nselec=0;selec[1]=0;selec[2]=0;selec[3]=0;selec[4]=0;
		nselec=0;
		mmol.deselecciona();
	}

	public void repinta(boolean b,boolean a,boolean h){
		if (b) this.bst=true;	else this.bst=false;
		if (a) this.atn=true;	else this.atn=false;
		if (h) this.hbk=true;	else this.hbk=false;
		this.repaint();
	}

	public void pintaCon(){autocon=true; repaint();}
	public void pintaAlt(){autocon=false;repaint();}

	public  void   setlrd(){this.lrd=true ;return;}
	public  void unsetlrd(){this.lrd=false;return;}
	public  void   setatn(){this.atn=true ;return;}
	public  void unsetatn(){this.atn=false;return;}
	public  void   sethbk(){this.hbk=true ;return;}
	public  void unsethbk(){this.hbk=false;return;}
	public  void   setbst(){this.bst=true ;return;}
	public  void unsetbst(){this.bst=false;return;}
}
