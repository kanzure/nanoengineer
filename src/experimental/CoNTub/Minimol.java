...
class Minimol
{
	...
	Minimol (MoleculaB mo)
	{
		...
		//Fast creation of one minimolecula for manipulation within a viewfinder 3D
		//From one molecula complex MoleculaB type.
	...
	// HABRIA THAT TO PUT TOOLS OF SELECTED
	...
	double angulo (int a, int b, int c)	//OJO EN GRADOS - angle in degrees
	{
		pto3D v = miniverts[a];
		pto3D w = miniverts[b];
		pto3D u = miniverts[c];
	...
	}				//OJO EN GRADOS - angle in degrees
	...
	void selecciona (int i, int status) // select (lit: "it selects")
	{
		miniselec[i] = status;
	}
	void deselecciona ()  // deselect
	{
		for (int i = 0; i < nvert; i++)
			miniselec[i] = 0;
	}
