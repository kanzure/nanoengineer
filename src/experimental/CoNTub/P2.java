...
import java.awt.event.*;import java.applet.*;import java.text.*;
...
public P2()
	...
	c0=new JPanel();	c0.setLayout(new BorderLayout());
	c1=new JPanel();	c1.setLayout(new GridLayout(4,2,5,5));
	c2=new JPanel();	c2.setLayout(new GridLayout(5,1,5,5));

	Box ct1=Box.createHorizontalBox();
	Box ct2=Box.createHorizontalBox();

	ct1.setBorder(BorderFactory.createCompoundBorder(
		BorderFactory.createTitledBorder("1st Tube:\nIndices and length. (\u212B)"),
		BorderFactory.createEtchedBorder(1)));
	ct2.setBorder(BorderFactory.createCompoundBorder(
		BorderFactory.createTitledBorder("2nd Tube:\nIndices and length. (\u212B)"),
		BorderFactory.createEtchedBorder(1)));

	txlog =new JTextArea("",6,15);ctxt2=new JScrollPane(txlog);
	txlog.setEditable(false);
	Font fon=new Font("Courier",1,12);
    txlog.setFont(fon);

	 B1    =new JButton("CREATE");	   B1.addActionListener(this);
	 B2    =new JButton("CREATE RANDOM");B2.addActionListener(this);

	 Box cv =Box.createVerticalBox();
	 cb1   =new JCheckBox("Ball & Stick (b)");cb1.addItemListener(this); bst=false;
	 cb2   =new JCheckBox("Atom labels (l)");cb2.addItemListener(this); atn=false;
	 cb3   =new JCheckBox("Cut back (c)");   cb3.addItemListener(this); hbk=false;
	 cv.add(cb1);
	 cv.add(cb2);
	 cv.add(cb3);
	...
	//int rand4=(int)(25*Math.random());
	 ...
	 l1    =new JLabel (" i= ");t1    =new JTextField ("5"); t1.addActionListener(this);
	 l2    =new JLabel (" j= ");t2    =new JTextField ("5") ; t2.addActionListener(this);
	 l3    =new JLabel (" l= ");t3    =new JTextField ("20"); t3.addActionListener(this);
	 l4    =new JLabel (" i= ");t4    =new JTextField ("10") ; t4.addActionListener(this);
	 l5    =new JLabel (" j= ");t5    =new JTextField ("10"); t5.addActionListener(this);
	 l6    =new JLabel (" l= ");t6    =new JTextField ("20"); t6.addActionListener(this);

	 ct1.add(l1);ct1.add(t1);ct1.add(l2);ct1.add(t2);ct1.add(l3);ct1.add(t3);
	 ct2.add(l4);ct2.add(t4);ct2.add(l5);ct2.add(t5);ct2.add(l6);ct2.add(t6);
	 ...
	 gl.redraw(Integer.parseInt(t1.getText().trim()),
	           ...
	           Integer.parseInt(t4.getText().trim()),
	           Integer.parseInt(t5.getText().trim()),true);
	 ...
	 add(ctxt2,BorderLayout.SOUTH );
     	...
     	ctxt2.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createTitledBorder("Generation Log."),
	 		                                            BorderFactory.createEtchedBorder(1)));
      HJ=new MoleculaT();
	 }
...
try{i1=Integer.parseInt(t1.getText().trim());
	j1=Integer.parseInt(t2.getText().trim());
	i2=Integer.parseInt(t4.getText().trim());
	j2=Integer.parseInt(t5.getText().trim());}

catch (Exception e){JOptionPane.showMessageDialog(null, "Stop: wrong input", "Stop", 0);  return;}
}
else if(etiq=="CREATE RANDOM")
{	 i1=(int)(25*Math.random());t1.setText(""+i1);
	 j1=(int)(25*Math.random());t2.setText(""+j1);
	 i2=(int)(25*Math.random());t4.setText(""+i2);
	 j2=(int)(25*Math.random());t5.setText(""+j2);
	}
else {i1=2;j1=2;i2=4;j2=4;}
 ...
 public void itemStateChanged(ItemEvent e) {
         Object source = e.getItemSelectable();
          if      (source==cb1){if   (e.getStateChange()==ItemEvent.DESELECTED) bst=false; else bst=true ;
			        pan3D.repinta(bst,atn,hbk); }
          else if (source==cb2){if   (e.getStateChange()==ItemEvent.DESELECTED) atn=false; else atn=true;
				pan3D.repinta(bst,atn,hbk); }
	  else if (source==cb3){if   (e.getStateChange()==ItemEvent.DESELECTED) hbk=false; else hbk=true;
			        pan3D.repinta(bst,atn,hbk); }
...
double lent1=0;double lent2=0;

try{ 	i1=Integer.parseInt(t1.getText().trim());//pillamos indice se tubos
	j1=Integer.parseInt(t2.getText().trim());
	i2=Integer.parseInt(t4.getText().trim());
	j2=Integer.parseInt(t5.getText().trim());
...
catch (NumberFormatException e){JOptionPane.showMessageDialog(null, "Stop: wrong input", "Stop", 0);  return;}
...
HJ.setInfo("("+i1+","+j1+")-("+i2+","+j2+") Nanotube Heterojunction with lengths "+lent1+" and "+lent2+" A.");
 ...
 if (i1==0&&j1==0){JOptionPane.showMessageDialog(null, "Error: 1st tube's indices are incorrect", "Error", JOptionPane.ERROR_MESSAGE);  return;}
 if (i2==0&&j2==0){JOptionPane.showMessageDialog(null, "Error: 2nd tube's indices are incorrect", "Error", JOptionPane.ERROR_MESSAGE);  return;}


 
 boolean cambio1=false;
 boolean cambio2=false;
 for (;(i1<0)||(j1<0);){int i1n=-j1;int j1n=i1+j1;i1=i1n;j1=j1n;cambio1=true;	}
 for (;(i2<0)||(j2<0);){int i2n=-j2;int j2n=i2+j2;i2=i2n;j2=j2n;cambio2=true;	}

 if (cambio1)
 {JOptionPane.showMessageDialog(null, "Warning: Indices of 1st tube automatically translated to ("+i1+","+j1+")", "Warning", JOptionPane.INFORMATION_MESSAGE);
 t1.setText(""+i1);t2.setText(""+j1);}
 if (cambio2)
 {JOptionPane.showMessageDialog(null, "Warning: Indices of 2nd tube automatically translated to ("+i2+","+j2+")", "Warning", JOptionPane.INFORMATION_MESSAGE);
 t4.setText(""+i2);t5.setText(""+j2);}


 if (i2==i1&&j2==j1)
 {JOptionPane.showMessageDialog(null, "Error: Indices of both tubes coincide", "Error", JOptionPane.ERROR_MESSAGE);  return;}
if (i2<2&&j2<2)
 {JOptionPane.showMessageDialog(null, "Stop: 2nd tube too narrow", "Stop", 0);  return;}
if (i1<2&&j1<2)
 {JOptionPane.showMessageDialog(null, "Stop: 1st tube too narrow", "Stop", 0);  return;}
...
int guess=naproxatomos(NTA.radio(),lent1,NTB.radio(),lent2);

if      (guess>6000){JOptionPane.showMessageDialog(null, "Structure will have more than 6000 Atoms. Process stopped.", "STOP",0);return;}
else if (guess>4000){int sale=JOptionPane.showConfirmDialog(null, "Structure will have more than 4000 Atoms. Continue at your own risk","Warning",0);if (sale==1) return;}
else if (guess>2000) {JOptionPane.showMessageDialog(null,"Large Structure: using low-consumption display.","Big molecule",1);pan3D.setlrd();}
else                 pan3D.unsetlrd();
 ...
 if (NTA.radio()<NTB.radio()) 	{creciente=true; c=-1; n=nai;m=mai;  td1="heptagon"; td2="pentagon"; }
 else 				{creciente=false; c=1; n=nad;m=mad;  td1="pentagon"; td2="heptagon"; }

 
 txlog.setText(txlog.getText()+"\nThe "+td1+" comes first, and the "+td2+" is at ("+n+","+m+"). ");

 if (!creciente ) txlog.setText(txlog.getText()+"The strip is ("+nad+","+mad+")[1,0];("+nbd+","+mbd+")[-1,0];");
 else             txlog.setText(txlog.getText()+"The strip is ("+nai+","+mai+")[1,0];("+nbi+","+mbi+")[-1,0];");
...
double A=2.46;double r3=Math.sqrt(3);
...
pto2D pD=new pto2D(A*(n+i2+(m+j2)*0.5) ,A*r3/2*(m+j2)	);
...
double maxcx=Math.max(Math.max(pA.x,pB.x),Math.max(pC.x,pD.x));
double maxcy=Math.max(Math.max(pA.y,pB.y),Math.max(pC.y,pD.y));
double mincx=Math.min(Math.min(pA.x,pB.x),Math.min(pC.x,pD.x));
double mincy=Math.min(Math.min(pA.y,pB.y),Math.min(pC.y,pD.y));
if (despleg)
{
...
for (double celx=stx;celx<=(maxcx-mincx)+4*A   ;celx=celx+A   ) 
    {pto2D at1=new pto2D(celx		,cely+A/r3);
     pto2D at2=new pto2D(celx		,cely-A/r3);
     pto2D at3=new pto2D(celx+A/2	,cely+A/2/r3);
     pto2D at4=new pto2D(celx+A/2	,cely-A/2/r3);
     if (at1.dentro4l(pB,pA,pC,pD)) {conoplano.addVert(at1,6); if (despleg) HJ.addVert(at1,6,Color.orange);      }//
     if (at2.dentro4l(pB,pA,pC,pD)) {conoplano.addVert(at2,6); if (despleg) HJ.addVert(at2,6,Color.orange);      }//
     if (at3.dentro4l(pB,pA,pC,pD)) {conoplano.addVert(at3,6); if (despleg) HJ.addVert(at3,6,Color.orange);      }//
     if (at4.dentro4l(pB,pA,pC,pD)) {conoplano.addVert(at4,6); if (despleg) HJ.addVert(at4,6,Color.orange);      }//
    }

pto2D pV=null;

double fc=0;
if (creciente)	{pV=new pto2D(A*(i1+j1-i1*0.5)		,-A*r3/2.0*i1	  );fc=-1; }
else 		{pV=new pto2D(A*(-j1+(i1+j1)*0.5)	, A*r3/2.0*(i1+j1));fc= 1; }
...
{pto3D ptp=conoplano.vert(i);
 ...
 double an;
 if (creciente) an=pA.menos(pV).a2D().angulocwhasta(ptp.menos(pV).a2D());
 else           an=pA.menos(pV).a2D().anguloccwhasta(ptp.menos(pV).a2D());
...
if (!despleg) if (!HJ.ocupa1(ptemp)) HJ.addVert(ptemp,6);
...
txlog.setText(txlog.getText()+"\n\nDetermination of both tubes. ");
 ...
 double anT2;
 if (creciente)  anT2=pA.menos(pV).a2D().angulocwhasta(pC.menos(pV).a2D());
 else            anT2=pA.menos(pV).a2D().anguloccwhasta(pC.menos(pV).a2D());
double dhT2=anT2*6.0;
txlog.setText(txlog.getText()+" Tubes 1 and 2 form a dihedral angle of "+(int)(dhT2*180./Math.PI)+" deg.");

//y creamos los vec
if (!creciente) {lt1= new pto3D( Math.sin(curvaP),                                              0,Math.cos(curvaP)                                   );
                 lt2= new pto3D( Math.sin(curvaH)*Math.cos(dhT2), Math.sin(curvaH)*Math.sin(dhT2),Math.cos(curvaH));}
else            {lt1= new pto3D(-Math.sin(curvaH),                                              0,Math.cos(curvaH)                                   );
...
pto2D pBcm1=new pto2D(A*(i1+j1*0.5)	,A*r3/2*j1	);
...
double maxcx1=Math.max(Math.max(pAcm1.x,pBcm1.x),Math.max(pCcm1.x,pDcm1.x));
double maxcy1=Math.max(Math.max(pAcm1.y,pBcm1.y),Math.max(pCcm1.y,pDcm1.y));
double mincx1=Math.min(Math.min(pAcm1.x,pBcm1.x),Math.min(pCcm1.x,pDcm1.x));
double mincy1=Math.min(Math.min(pAcm1.y,pBcm1.y),Math.min(pCcm1.y,pDcm1.y));
...
for (int celx=stx1;celx<=enx1;celx++) //recorrido horizontal
    {pto2D at1=new pto2D(celx*A		,cely*A*r3+A/r3);
     pto2D at2=new pto2D(celx*A		,cely*A*r3-A/r3);
     pto2D at3=new pto2D(celx*A+A/2.0	,cely*A*r3+A/2.0/r3);
     pto2D at4=new pto2D(celx*A+A/2.0	,cely*A*r3-A/2.0/r3);
     if (at1.dentro4l(pAcm1,pBcm1,pDcm1,pCcm1)) {cachotubo1.addVert(at1,6);posis1[0][indi1]=1;posis1[1][indi1]=celx-cely  ;posis1[2][indi1]=cely*2  ;indi1++; if (despleg) HJ.addVert(at1,6,Color.blue);}//
     if (at2.dentro4l(pAcm1,pBcm1,pDcm1,pCcm1)) {cachotubo1.addVert(at2,6);posis1[0][indi1]=2;posis1[1][indi1]=celx-cely+1;posis1[2][indi1]=cely*2-2;indi1++; 	if (despleg) HJ.addVert(at2,6,Color.blue);}//
     if (at3.dentro4l(pAcm1,pBcm1,pDcm1,pCcm1)) {cachotubo1.addVert(at3,6);posis1[0][indi1]=2;posis1[1][indi1]=celx-cely+1;posis1[2][indi1]=cely*2-1;indi1++; 	if (despleg) HJ.addVert(at3,6,Color.blue);}//
     if (at4.dentro4l(pAcm1,pBcm1,pDcm1,pCcm1)) {cachotubo1.addVert(at4,6);posis1[0][indi1]=1;posis1[1][indi1]=celx-cely+1;posis1[2][indi1]=cely*2-1;indi1++; 	if (despleg) HJ.addVert(at4,6,Color.blue);}//
     }
...
pto2D puntoproy=ptp.proyeccplano(pTcm1corto).a2D();
 ...
 double an;
 if (creciente) {        an=pA.menos(pV).a2D().angulocwhasta (puntoproy.menos(pV).a2D());}
 else           {        an=pA.menos(pV).a2D().anguloccwhasta(puntoproy.menos(pV).a2D());}
...
if      (diz<GMI) co=0.0; 
else if (diz>GAP) co=1.0; 
else             co=(diz-GMI)/(GAP-GMI); 
...
if (!despleg) if (!HJ.ocupa1(pperf)) HJ.addVert(pperf,6,Color.gray);
...
//TEST
if (vecs)
{pto3D pbaseO=aproxNT(new pto3D(0,0,0), 1,0,0,pstart1,i1,j1,lt1,new pto3D(1,0,0),1);
pto3D pbase1=aproxNT(new pto3D(0,0,0), 1,1,0,pstart1,i1,j1,lt1,new pto3D(1,0,0),1);
pto3D pbase2=aproxNT(new pto3D(0,0,0), 1,0,1,pstart1,i1,j1,lt1,new pto3D(1,0,0),1);
HJ.addVert(pbaseO,6);
HJ.addVert(pbase1,7);
HJ.addVert(pbase2,8);
HJ.conecta(HJ.nvert()-1,HJ.nvert()-3);
...
pto2D pTcm2=new pto2D(-(pBcm2.y-pAcm2.y),pBcm2.x-pAcm2.x);
pto3D pTcm2corto=pTcm2.aversor().escala(lent2);
pto2D pCcm2=pAcm2.mas(pTcm2corto).a2D();
pto2D pDcm2=pBcm2.mas(pTcm2corto).a2D();

double maxcx2=Math.max(Math.max(pAcm2.x,pBcm2.x),Math.max(pCcm2.x,pDcm2.x));
double maxcy2=Math.max(Math.max(pAcm2.y,pBcm2.y),Math.max(pCcm2.y,pDcm2.y));
double mincx2=Math.min(Math.min(pAcm2.x,pBcm2.x),Math.min(pCcm2.x,pDcm2.x));
double mincy2=Math.min(Math.min(pAcm2.y,pBcm2.y),Math.min(pCcm2.y,pDcm2.y));
...
for (int celx=stx2;celx<=enx2;celx++) //recorrido horizontal
    {pto2D at1=new pto2D(celx*A		,cely*A*r3+A/r3);
     ...
     {cachotubo2.addVert(at1,6);posis2[0][indi2]=1;posis2[1][indi2]=celx-cely  ;posis2[2][indi2]=cely*2  ;indi2++; 	if (despleg) HJ.addVert(at1,6,Color.red);} //
     if (at2.dentro4l(pBcm2,pAcm2,pCcm2,pDcm2))
     {cachotubo2.addVert(at2,6);posis2[0][indi2]=2;posis2[1][indi2]=celx-cely+1;posis2[2][indi2]=cely*2-2;indi2++; 	if (despleg) HJ.addVert(at2,6,Color.red);} //
     if (at3.dentro4l(pBcm2,pAcm2,pCcm2,pDcm2))
     {cachotubo2.addVert(at3,6);posis2[0][indi2]=2;posis2[1][indi2]=celx-cely+1;posis2[2][indi2]=cely*2-1;indi2++; 	if (despleg) HJ.addVert(at3,6,Color.red);} //
     if (at4.dentro4l(pBcm2,pAcm2,pCcm2,pDcm2))
     {cachotubo2.addVert(at4,6);posis2[0][indi2]=1;posis2[1][indi2]=celx-cely+1;posis2[2][indi2]=cely*2-1;indi2++; 	if (despleg) HJ.addVert(at4,6,Color.red);} //
     }
...
if      (diz<GMI) co=0.0; 
else if (diz>GAP) co=1.0; //ES el Gap de AProximacion;
else             co=(-diz+GMI)/(-GAP+GMI); 
...
if (vecs){
pto3D pbaseO2=aproxNT(new pto3D(0,0,0), 1,0,0,pstart2,i2,j2,lt2,pori2,1);
pto3D pbase12=aproxNT(new pto3D(0,0,0), 1,1,0,pstart2,i2,j2,lt2,pori2,1);
pto3D pbase22=aproxNT(new pto3D(0,0,0), 1,0,1,pstart2,i2,j2,lt2,pori2,1);
...
txlog.setText(txlog.getText()+"\n\nStructure ("+i1+","+j1+")-("+i2+","+j2+") completed with "+HJ.nvert()+" atoms");
}
else
{
...
pto2D Qad=new pto2D(A*(nad + mad*0.5)	,A*r3/2* mad 	);
pto2D Qai=new pto2D(A*(nai + mai*0.5)	,A*r3/2* mai 	);

pto2D pA1 = new pto2D(0,0);
pto2D pB1 = new pto2D(A*(i1+j1*0.5)	,A*r3/2*j1	);
...
if      ((angulod<90.)&&(anguloi>90.)) {directo=true; n=nad;m=mad;
}
else if ((anguloi<90.)&&(angulod>90.)) {directo=false; n=nai;m=mai;
}
else    JOptionPane.showMessageDialog(null, "Warning: Unexpected geometry!!", "Warning", JOptionPane.INFORMATION_MESSAGE);

pto2D pC1 = new pto2D(A*(n + m*0.5)	,A*r3/2* m 	);

pto2D pA2 = new pto2D(0,0);
pto2D pB2 = new pto2D(A*(i2+j2*0.5)	,A*r3/2*j2	);

int nprima=0;
int mprima=0;
if (directo   ) {nprima=-m ;mprima=n+m;}
else            {nprima=n+m;mprima=-n ;}

pto2D pC2 = new pto2D(A*(nprima + mprima*0.5)	,A*r3/2* mprima 	);
...
double maxcx1=Math.max(Math.max(pA1.x,pB1.x),Math.max(pA1b.x,pB1b.x));
double maxcy1=Math.max(Math.max(pA1.y,pB1.y),Math.max(pA1b.y,pB1b.y));
double mincx1=Math.min(Math.min(pA1.x,pB1.x),Math.min(pA1b.x,pB1b.x));
double mincy1=Math.min(Math.min(pA1.y,pB1.y),Math.min(pA1b.y,pB1b.y));

double maxcx2=Math.max(Math.max(pA2.x,pB2.x),Math.max(pA2b.x,pB2b.x));
double maxcy2=Math.max(Math.max(pA2.y,pB2.y),Math.max(pA2b.y,pB2b.y));
double mincx2=Math.min(Math.min(pA2.x,pB2.x),Math.min(pA2b.x,pB2b.x));
double mincy2=Math.min(Math.min(pA2.y,pB2.y),Math.min(pA2b.y,pB2b.y));
...
for (int celx=stx1;celx<=enx1;celx++) //recorrido horizontal
    {pto2D at1=new pto2D(celx*A		,cely*A*r3+A/r3);
     ...
     pto2D at3=new pto2D(celx*A+A/2.0	,cely*A*r3+A/2.0/r3);
     pto2D at4=new pto2D(celx*A+A/2.0	,cely*A*r3-A/2.0/r3);
     if (at1.dentro4l(pA1,pC1,pC1b,pA1b)||at1.dentro4l(pC1,pB1,pB1b,pC1b)) {cachot1.addVert(at1,6);posis1[0][indi1]=1;posis1[1][indi1]=celx-cely  ;posis1[2][indi1]=cely*2  ;indi1++; 	}//HJ.addVert(at1,7);
     if (at2.dentro4l(pA1,pC1,pC1b,pA1b)||at2.dentro4l(pC1,pB1,pB1b,pC1b)) {cachot1.addVert(at2,6);posis1[0][indi1]=2;posis1[1][indi1]=celx-cely+1;posis1[2][indi1]=cely*2-2;indi1++; 	}//HJ.addVert(at2,7);
     if (at3.dentro4l(pA1,pC1,pC1b,pA1b)||at3.dentro4l(pC1,pB1,pB1b,pC1b)) {cachot1.addVert(at3,6);posis1[0][indi1]=2;posis1[1][indi1]=celx-cely+1;posis1[2][indi1]=cely*2-1;indi1++; 	}//HJ.addVert(at3,7);
     if (at4.dentro4l(pA1,pC1,pC1b,pA1b)||at4.dentro4l(pC1,pB1,pB1b,pC1b)) {cachot1.addVert(at4,6);posis1[0][indi1]=1;posis1[1][indi1]=celx-cely+1;posis1[2][indi1]=cely*2-1;indi1++; 	}//HJ.addVert(at4,7);
    }
...
for (int celx=stx2;celx<=enx2;celx++) //recorrido horizontal
    {pto2D at1=new pto2D(celx*A		,cely*A*r3+A/r3);
     ...
     pto2D at3=new pto2D(celx*A+A/2.0	,cely*A*r3+A/2.0/r3);
     pto2D at4=new pto2D(celx*A+A/2.0	,cely*A*r3-A/2.0/r3);
     if (at1.dentro4l(pA2,pA2b,pC2b,pC2)||at1.dentro4l(pC2,pC2b,pB2b,pB2)) {cachot2.addVert(at1,6);posis2[0][indi2]=1;posis2[1][indi2]=celx-cely  ;posis2[2][indi2]=cely*2  ;indi2++;} //HJ.addVert(at1,1);
     if (at2.dentro4l(pA2,pA2b,pC2b,pC2)||at2.dentro4l(pC2,pC2b,pB2b,pB2)) {cachot2.addVert(at2,6);posis2[0][indi2]=2;posis2[1][indi2]=celx-cely+1;posis2[2][indi2]=cely*2-2;indi2++; 	} //HJ.addVert(at2,1);
     if (at3.dentro4l(pA2,pA2b,pC2b,pC2)||at3.dentro4l(pC2,pC2b,pB2b,pB2)) {cachot2.addVert(at3,6);posis2[0][indi2]=2;posis2[1][indi2]=celx-cely+1;posis2[2][indi2]=cely*2-1;indi2++; 	} //HJ.addVert(at3,1);
     if (at4.dentro4l(pA2,pA2b,pC2b,pC2)||at4.dentro4l(pC2,pC2b,pB2b,pB2)) {cachot2.addVert(at4,6);posis2[0][indi2]=1;posis2[1][indi2]=celx-cely+1;posis2[2][indi2]=cely*2-1;indi2++; 	} //HJ.addVert(at4,1);
    }
...
{pto3D p=((Atomo)cachot1.susatomos.get(i)).vert;
 ...
 if (p.angulocong(pA1b)<=90.0) yg*=-1;  
...
double co=0.0;
if      (htolq>-GMI) co=0.0; //ES el Gap MInimo
else if (htolq<-GAP) co=1.0; //ES el Gap de AProximacion;
else             co=(zf+GMI)/(-GAP+GMI); //num siempre creciente, den negativo
...
pto3D pperf=aproxNTchap(ptemp,      //Punto a aproximar
			...
			new pto3D(R,0,0),
			co);		
if (!HJ.ocupa1(pperf)) HJ.addVert(pperf,6);
}
...
{pto3D p=((Atomo)cachot2.susatomos.get(i)).vert;
 ...
 if (p.angulocong(pA2b)>=90.0) yg*=-1;  //los que son negativos
...
if (!HJ.ocupa1(pperf)) HJ.addVert(pperf,6,Color.gray);
}
...
if      (combo.getSelectedItem()=="Hydrogen") {HJ.cierraH();HJ.ponconec(1.35);}
else if (combo.getSelectedItem()=="Nitrogen") {HJ.cierraN();HJ.ponconec(1.35);}
HJ.centrar();
pan3D.cargarMol(HJ);
pan3D.repaint();
...
public MoleculaT molT(){return HJ;}
...
double a1=v1.angulocwhasta(v2);	 if (a1<0||a1>Math.PI) nconcav++;
double a2=v2.angulocwhasta(v3);	 if (a2<0||a2>Math.PI) nconcav++;
double a3=v3.angulocwhasta(v4);	 if (a3<0||a3>Math.PI) nconcav++;
double a4=v4.angulocwhasta(v1);	 if (a4<0||a4>Math.PI) nconcav++;
...
int naproxatomos(double r1,double l1,double r2,double l2)
 ...
 double rr2=6.0*r2;
 double acono=0.;
 if (r1>r2) acono=Math.PI*(rr1*rr1-rr2*rr2)/6.0;
 else       acono=Math.PI*(rr2*rr2-rr1*rr1)/6.0;
