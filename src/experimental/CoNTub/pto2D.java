...
pto2D() {x=0.0f;y=0.0f;z=0.0f;}
pto2D(float xt, float yt) {x=xt;y=yt;z=0.0f;}
pto2D(double xt, double yt) {x=(float)xt;y=(float)yt;z=0.0f;}
...
boolean dentro3(pto2D A,pto2D B, pto2D C) //Ha de ser un metodo estricto, aunque se ponen >=
	...
	if  ((anguloP1<=angulo1)&&(anguloP2<=angulo2)) return true;
	else return false;
	}
...
//      A-----------------------B
	...
	if  ((anguloPbisA<anguloA)&&(anguloPbisD<anguloD)) return true;
	else return false;
...
if (a1<margen||a1>Math.PI-margen) {nconcav++;ac[0]=true;} else ac[0]=false;
if (a2<margen||a2>Math.PI-margen) {nconcav++;ac[1]=true;} else ac[1]=false;
if (a3<margen||a3>Math.PI-margen) {nconcav++;ac[2]=true;} else ac[2]=false;
if (a4<margen||a4>Math.PI-margen) {nconcav++;ac[3]=true;} else ac[3]=false;
...
if      (nconcav==0) {	//Convexo, usamos el anterior
			...
			int ocon1=-1;
			for (int i=0;i<4;i++){if (ac[i]==true) ocon1=i; }
			int ocona=ocon1+1;if (ocona>3) ocona-=4;
			int ocon2=ocon1+2;if (ocon2>3) ocon2-=4;
			int oconb=ocon1+3;if (oconb>3) oconb-=4;
			boolean dentroa=dentro3l(ap[ocon1],ap[ocona],ap[ocon2]); //EXPERIMENTAL!!
			boolean dentrob=dentro3l(ap[ocon1],ap[oconb],ap[ocon2]);
			if (dentroa||dentrob) return true;
			else return false;
			}
			...
			int conv1=-1;
			int conv2=-1;
			for (int i=0;i<4;i++){if (ac[i]==false) conc1=i; }
			//Ya tenemos el primero, buscamos el segundo con concavidad
			//van por parejas, conc, conc, no conc, no conc, nunca alternados
			//Buscamos el segundo
			for (int i=0;i<4;i++){if ((i!=conc1)&&(ac[i]==false)) conc2=i; }
			//Una vez que conocemos los dos concavos, buscamos los convexos
			for (int i=0;i<4;i++){if (ac[i]==true) conv1=i; }
			for (int i=0;i<4;i++){if ((i!=conv1)&&(ac[i]==true)) conv2=i; }
			//ahora, asociamos a cada puntillo el vecino con concavidad contraria
			int cc1v1=conc1+1;if (cc1v1>3) cc1v1-=4;
			int cc1v2=conc1+3;if (cc1v2>3) cc1v2-=4;
			int cc2v1=conc2+1;if (cc2v1>3) cc2v1-=4;
			int cc2v2=conc2+3;if (cc2v2>3) cc2v2-=4;
			int cv1v1=conv1+1;if (cv1v1>3) cv1v1-=4;
			int cv1v2=conv1+3;if (cv1v2>3) cv1v2-=4;
			int cv2v1=conv2+1;if (cv2v1>3) cv2v1-=4;
			int cv2v2=conv2+3;if (cv2v2>3) cv2v2-=4;
			//Ya tenemos a cada uno con sus vecinos. Cual es el de concav opuesta?
			int cc1op=-1;
			if      ((ac[cc1v1]!=ac[conc1])&&(ac[cc1v2]==ac[conc1])) cc1op=cc1v1;
			else if ((ac[cc1v2]!=ac[conc1])&&(ac[cc1v1]==ac[conc1])) cc1op=cc1v2;
			int cc2op=-1;
			if      ((ac[cc2v1]!=ac[conc2])&&(ac[cc2v2]==ac[conc2])) cc2op=cc2v1;
			else if ((ac[cc2v2]!=ac[conc2])&&(ac[cc2v1]==ac[conc2])) cc2op=cc2v2;
			int cv1op=-1;
			if      ((ac[cv1v1]!=ac[conv1])&&(ac[cv1v2]==ac[conv1])) cv1op=cv1v1;
			else if ((ac[cv1v2]!=ac[conv1])&&(ac[cv1v1]==ac[conv1])) cv1op=cv1v2;
			int cv2op=-1;
			if      ((ac[cv2v1]!=ac[conv2])&&(ac[cv2v2]==ac[conv2])) cv2op=cv2v1;
			else if ((ac[cv2v2]!=ac[conv2])&&(ac[cv2v1]==ac[conv2])) cv2op=cv2v2;

boolean ta=dentrocasi3(ap[conc1],ap[conc2],ap[cc1op].menos(ap[conc1]),ap[cc2op].menos(ap[conc2]));
boolean tb=dentrocasi3(ap[conv1],ap[conv2],ap[cv1op].menos(ap[conv1]),ap[cv2op].menos(ap[conv2]));
...
else if (nconcav==3) {	int ocon1=-1;
			for (int i=0;i<4;i++){if (ac[i]==false) ocon1=i; }
			int ocona=ocon1+1;if (ocona>3) ocona-=4;
			int ocon2=ocon1+2;if (ocon2>3) ocon2-=4;
			int oconb=ocon1+3;if (oconb>3) oconb-=4;
...
boolean dentro4(pto2D A,pto2D B, pto2D C, pto2D D){ return dentro4(A,B,C,D,0.0);}
...
boolean dentro4e(pto2D A,pto2D B, pto2D C, pto2D D){return dentro4(A,B,C,D,1.00);} //estricto

boolean dentro4l(pto2D A,pto2D B, pto2D C, pto2D D){return dentro4(A,B,C,D,1.01);} //laxo


pto2D mas(  pto2D p){return new pto2D(this.x+p.x,this.y+p.y);}
pto2D menos(pto2D p){return new pto2D(this.x-p.x,this.y-p.y);}
}
