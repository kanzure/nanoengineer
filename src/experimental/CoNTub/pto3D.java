class pto3D {
	...
	void giroxg(double thetag,pto3D pau){ giroxr(thetag*(Math.PI / 180),pau);
	}  //GRADOS
	...
	void giroyg(double thetag,pto3D pau){ giroyr(thetag*(Math.PI / 180),pau);
	}  //GRADOS
	...
	void girozg(double thetag,pto3D pau){ girozr(thetag*(Math.PI / 180),pau);
	}  //GRADOS
	...
	double modulo(){
		double mod2=this.prodesc(this);return Math.sqrt(mod2);
	}
	...
	{ //ESTABAMOS CREANDO UN VERSOR MEDIO PONDERADO DE 0 A 1
		...
		if (param<0) {
			...
			pto3D cone=v1.menos(this); salida=this.mas(cone.escala(param));
		}
		return salida;
	}
