class pto3D {
    ...
    pto3D () {	x = 0.0f;	y = 0.0f;	z = 0.0f;   }
    pto3D (double X,double Y,double Z) {x=(float)X; y=(float)Y; z=(float)Z; }
...
OK  dihedro(pto2 pto1) del punto en cuestion a traves de p2 medido respecto p1
	...
	y = Ny;	z = Nz;    }
    void giroxg(double thetag){giroxr(thetag*(Math.PI / 180));}  //GRADOS
	...
	x = Nx;	z = Nz;    }
    void giroyg(double thetag){giroyr(thetag*(Math.PI / 180));}  //GRADOS
	...
	x = Nx;	y = Ny;    }
    void girozg(double thetag){girozr(thetag*(Math.PI / 180));}  //GRADOS
...
//METODOS RAPIDOS para los giros del visualizador, con cosenos y senos directore
   ...
   /** rotate theta degrees about the x axis */
	...
	return sal ;  }
    pto3D ngiroxg(double thetag){return ngiroxr(thetag*(Math.PI / 180));}  //GRADOS
	...
	return sal ;    }
    pto3D ngiroyg(double thetag){return ngiroyr(thetag*(Math.PI / 180));}  //GRADOS
	...
	return sal;     }
    pto3D ngirozg(double thetag){return ngirozr(thetag*(Math.PI / 180));}  //GRADOS

    /** rotate theta degrees about the x axis y cierto punto*/
    void giroxr(double theta,pto3D paux) {
	double ct = Math.cos(theta);
	double st = Math.sin(theta);
	x=x-paux.x;y=y-paux.y;z=z-paux.z;
	float Ny = (float) (y * ct + z * st);
	float Nz = (float) (-y* st + z * ct);
	y = Ny;	z = Nz;
	x=x+paux.x;y=y+paux.y;z=z+paux.z;
	}
    void giroxg(double thetag,pto3D pau){ giroxr(thetag*(Math.PI / 180),pau);}  //GRADOS
	...
	x = Nx;	z = Nz;
	x=x+paux.x;y=y+paux.y;z=z+paux.z;}
    void giroyg(double thetag,pto3D pau){ giroyr(thetag*(Math.PI / 180),pau);}  //GRADOS
    ...
    void girozg(double thetag,pto3D pau){ girozr(thetag*(Math.PI / 180),pau);}  //GRADOS
...
pto3D ngirag(double thetag,pto3D pau){return ngirar(thetag*(Math.PI / 180),pau);}  //GRADOS
...
pto3D escala(double factor){
		pto3D res=new pto3D(this.x*factor,this.y*factor,this.z*factor);
		return res;}
...
double angulocong(pto3D pto2){return anguloconr(pto2)*180.0/Math.PI;}
...
double modulo(){double mod2=this.prodesc(this);return Math.sqrt(mod2);}
...
double dihedror(pto3D ptoc,pto3D pto1){	//ACLARACION dihedro positivo es cuando, mirando en el sentido marcado
...
double dihedrog(pto3D ptoc,pto3D pto1){return dihedror(ptoc,pto1)*180.0/Math.PI;}
...
void versoriza(){double mod=this.modulo();
...
pto3D ptopondcon(pto3D v1,double param)
	...
	pto3D salida=null;
	if      (param<0) salida=this.clona();
	else if (param>1) salida=  v1.clona();
	else    {pto3D cone=v1.menos(this); salida=this.mas(cone.escala(param));}
	return salida;
	}

pto2D a2D(){return new pto2D(x,y);}
