...
class MoleculaB {
	...
	//A REHACER!!
	void addVert(pto3D punto,int ti,Color c){susatomos.add(new Atomo(punto,ti,c));}

	void addVert(pto3D punto,int ti){susatomos.add(new Atomo(punto,ti));}

	void addVert(double x, double y, double z, int ti ) {susatomos.add(new Atomo(new pto3D(x,y,z),ti))   ;  }
	...
	double getLejania(){
		...
		for (int i = 0; i<susatomos.size();i++) {
			...
			if (dist>lej) lej=dist;
		}
		return lej;
	}

	void vaciar (){
		susatomos = new  ArrayList(0);  TablaP=new tabPe();
	}

	//HABRIA QUE PONER HERRAMIENTAS DE SELECCIONADO
	void deseleccionar() {
		for (int i = 0; i<susatomos.size();i++)
			((Atomo)susatomos.get(i)).selec=0; nselec=0;
	}


	void centrar() {
		if (susatomos.size() <= 0) return;
		double x=0,y=0,z=0;
	...
	void giroxr(double th){for (int i=0; i<susatomos.size();i++){((Atomo)susatomos.get(i)).vert.giroxr(th);}}
	void giroyr(double th){for (int i=0; i<susatomos.size();i++){((Atomo)susatomos.get(i)).vert.giroyr(th);}}
	void girozr(double th){for (int i=0; i<susatomos.size();i++){((Atomo)susatomos.get(i)).vert.girozr(th);}}

	void giroxg(double th){giroxr(th/180.0*Math.PI);}
	void giroyg(double th){giroyr(th/180.0*Math.PI);}
	void girozg(double th){girozr(th/180.0*Math.PI);}
	...
	void rm(int n){
		if (n>susatomos.size()-1) return; else susatomos.remove(n);
	} //elimina elemento n (solo hasta size-1)

	void rmlast(int n) {
		for (int i=1;i<=n;i++) this.rmlast();
	...
	int atomoqueocupa1(pto3D pto){
		int ocup=0;
		for (int i=0;i<susatomos.size();i++){pto3D pto2=((Atomo)susatomos.get(i)).vert;
		if (pto.dista(pto2)<0.5) ocup=i;	}
		return ocup;
	}
	...
	int atomoqueocupa(pto3D pto,double limite){
		int ocup=0;
		for (int i=0;i<susatomos.size();i++){pto3D pto2=((Atomo)susatomos.get(i)).vert;
		if (pto.dista(pto2)<limite) ocup=i;	}
		return ocup;
	}
	...
	int atomoqueocupa2(pto3D pto){
		...
		for (int i=0;i<susatomos.size();i++)
		{pto3D pto2=((Atomo)susatomos.get(i)).vert;
		if (pto.dista(pto2)<1.3) ocup=i;	}
		return ocup;
	}

	void mueve(int num,double x, double y, double z){
		if (num<susatomos.size()) ((Atomo)susatomos.get(num)).vert=new pto3D(x,y,z);
	}     //OJO A LAS SUSTITUCIONES //ANTES ERA SUSTITUYE

	void mueve(int s, pto3D pto){ if (s<susatomos.size()) ((Atomo)susatomos.get(s)).vert=pto;
	}

	void sustituye(int num,int ti, pto3D pto){if (num<=susatomos.size()) susatomos.set(num,new Atomo(pto,ti));
	}
	//ESTA es una autentica sustitucion

	void ponconec(){ponconec(1.30);
	}
	...
	void reconec(){reconec(1.30);
	}

	void reconec(double param){ //IGUAL QUE PONCONEC; pero sin iniciar
		int nv=susatomos.size();
		for (int i=0;i<nv;i++)
		{
			pto3D ptoa=((Atomo)susatomos.get(i)).vert;
			...
			for (int j=1;j<=mc[0];j++) {
				...
				for (int k=1;k<=mc[0];k++) {
					...
					if ((angulo<60)&&(angulo>1)) {
						if (pb.dista(pa)<pc.dista(pa)) desconecta(i,mc[k]);
						else                           desconecta(i,mc[j]);
						dc++;
					}
				}
			}
	...
	int nvec (int n)   //NUMERO DE VECINOS
	{
		...
		for (int j=0; j<susatomos.size();j++) {
			...
			if (pto.dista(p)<1.6&&pto.dista(p)>0.1) nv++;
		}
		return nv;
 	}


	//Centra un atomo en su entorno de tres vecinos
	...
	void conecta(int i, int j) {
		...
		for (int k=1;k<=ni;k++)
			if (mci[k]==j) yaconi=true;
		if (yaconi==false) {
			mci[0]=ni+1;mci[ni+1]=j;
		}

		boolean yaconj=false;
		for (int k=1;k<=nj;k++)
			if (mcj[k]==i) yaconj=true;
		if (yaconi==false) {
			mcj[0]=nj+1;mcj[nj+1]=i;
		}
	...
	//Metodo alternativo para la matriz z
	...
	void desconecta(int i, int j) {
		...
		for (int k=1;k<=ni;k++) {
			if (j==mci[k]) {
				for (int l=k;l<=ni-1;l++) {mci[l]=mci[l+1];
				}
				mci[ni]=0;
				mci[0 ]=ni-1;
				ni--;
		...
		for (int k=1;k<=nj;k++) {
			if (i==mcj[k]) {
				for (int l=k;l<=nj-1;l++) {mcj[l]=mcj[l+1];
				}
				mcj[nj]=0;
				mcj[0 ]=nj-1;
				nj--;
