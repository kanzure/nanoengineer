class anillo {  // ring
	...
	pto3D centroide;
	int vert[];
	anillo() { this.num=0;centroide=null;vert =new int[15]; }
	...
	/*Resumen de operaciones
	  ...
	  to add element
	  turn of numbers  (number of turns??)
	  to order anticlockwise
	  change origin
	*/

	void addVert(int pton) {
		if (this.num<14){
			this.num++;this.vert[num]=pton;
		}
	}

	void setCentro(pto3D cide){this.centroide=cide;  }
	...
	void centracentroide(MoleculaT mol)
	{
		...
		ncide.escala(1/num);
		if (ncide.dista(this.centroide)<0.7) centroide=ncide;
	}


	//vector de referencia y molecula a la que pertenecen (con toda  informacion 3D)
	// vector of reference and molecule to which it belongs (with all information 3D)
	void ordena(pto3D vecref,MoleculaT mol)
	{
		...
		//we created a vector that indicates where is the outside of the tube, and from where we watched
		...
		//We looked for atom but near by the right
		int dcha=0;double dis=10;
		for (int i=1;i<=num;i++) {
			...
			if (di<dis && dh >30 && dh<150){dis=di; dcha=i;}
		}
		...
		for (int i=2;i<=num;i++){
			...
			for (int j=1;j<=num;j++){
				...
				if (dist<distan&&dist>0.01 && dhdr >-150 && dhdr<-30) {
					distan=dist; nizq=j; dihedrotemp=dhdr;
				}
			}
			newvert[i]=vert[nizq];
		}
		for (int i=1;i<=num;i++) vert[i]=newvert[i];
	}
	...
	//pasa los indices hacia abajo(si giro>0), con lo que se cambia la orientación ccw)
	//it passes the indices downwards (if turn > 0), with which ccw changes orientation
	void rota(int giro)
	{
		int newvert[]=new int[num+1];
		for (int i=1;i<=num;i++)
		{
			int nuevapos=i-giro;
			if      (nuevapos>num  && nuevapos<2*num) nuevapos=nuevapos-num;
			else if (nuevapos<=0   && nuevapos> -num) nuevapos=nuevapos+num;
	...
	String aCadena()
	{
		String cad="Miembros: ";  // members
		for (int i=1;i<=num;i++) cad=cad+" "+vert[i];
		if (centroide!=null) cad=cad+" centroide en "+centroide.aTexto();
