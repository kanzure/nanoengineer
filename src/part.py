...
class Part(InvalMixin):
    ...
    def __init__(self, assy, topnode):
        ...
        if topnode.part:
            # this part might be destroyed when we add topnode to ourselves,
            # so we'd better first salvage from it whatever might be useful
            # (copying its view attributes fixes bug 556 [bruce 050420])
            self.homeCsys = topnode.part.homeCsys.copy()
            self.lastCsys = topnode.part.lastCsys.copy()
            ...
            if topnode.part.name and not topnode.part.topnode:
                # steal its name
                self.name = topnode.part.name
                topnode.part.name = None
        else:
