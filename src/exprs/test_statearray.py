...
class xxx_drag_behavior(DragBehavior): # revised 070316
    ...
    ### drag event object can be passed to us... as a delegate! [In recent code, it seems like the Highlightable is taking this role.]
...
# UNFINISHED aspects of that:
# + implem posn_from_params
...
# Now it is supposedly coded enough to work using initial kluge test, but it doesn't work,
# but is making 2 highlightables and showing one and trying to drag the other (and failing). No idea why.
...
class _height_dragger(DelegatingInstanceOrExpr):
    ...
    ########REVIEW: the following CAN'T WORK (I think -- not sure) since it is trying to set up a cyclic inter-Instance ref
    # using initializers! But I am not sure, since maybe things are still symbolic when the objs are made, then evalled later.
    # So it might actually work, in fact I am now guessing it will. Or at least it might and it's worth a try.
    drag_handler = Instance( xxx_drag_behavior( _self.delegate, height_ref, Ray(ORIGIN, DX) )) ### DX for initial test, then DZ
