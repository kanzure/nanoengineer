...
class Assembly( StateMixin, Assembly_API):
    ...
    def change_indicator_modified_since_last_call(self, 
                                                  indicator = ''): #Ninad080818
        """
        Returns True if the '*_change_indicator' changed since this method was 
        last called. 
        
        Example: When this method was last called, the 
        selection_change_indicator value was 10 (for instance). When the caller 
        called this method again, the selection_change_indicator value was 
        now 12. This information may be used by model_changed() method in 
        commands or their PM.         
               
        @param indicator: The caller wants to know if this change indicator 
                          was changed since last call of this method. 
        @type  indicator: string
        @see: self.model_change_indicator()
        @see: self.all_change_indicators()
        @see: self.view_change_indicator()
        
        """
        #NOTE: the method is NOT YET USED as of 2008-08-18
        #Tentative implementation, may be revised after review. The method 
        #name might also be changed. 
        
        changed = False
        
        try:
            change_indicator_method = getattr(self, indicator)
        except:
            msg = "bug: %s doesn't have an " \
                  "attr named %r" % (self, indicator)
            print_compact_traceback( msg + ": " )
            return 
        
        attr_name  = '_previous_' + str(indicator)
        try:
            previous_change_indicator = getattr(self, attr_name)
        except:
            msg = "bug: %s doesn't have an " \
                  "attr named %r" % (self, previous_change_indicator)
            print_compact_traceback( msg + ": " )
            return 
            
        
        changed = not same_vals(change_indicator_method(), 
                            previous_change_indicator) 
        previous_change_indicator = change_indicator_method()
        
        ##if 0:
            ##if indicator == 'all_change_indicators':
                ##changed = not same_vals(self.all_change_indicators(), 
                                 ##self._previous_all_change_indicators)
                ##self._previous_all_change_indicators = self.all_change_indicators()
            ##elif indicator == 'selection_change_indicator':
                ##changed = not same_vals(self.selection_change_indicator(), 
                                 ##self._previous_selection_change_indicator)
                ##self._previous_selection_change_indicator = self.selection_change_indicator()
            ##elif indicator == 'model_change_indicator':
                ##changed = not same_vals(self.model_change_indicator(), 
                                 ##self._previous_model_change_indicator)
                ##self._previous_model_change_indicator = self.model_change_indicator()
            ##elif indicator == 'view_change_indicator':
                ##changed = not same_vals(self.view_change_indicator(), 
                                 ##self._previous_view_change_indicator)
                ##self._previous_view_change_indicator = self.view_change_indicator
            ##else:
                ##print_compact_stack("error: requested indicator %s doesn't exist"%indicator)
