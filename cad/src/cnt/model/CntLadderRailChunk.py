...
def _DEBUG_REUSE_CHUNKS():
    return debug_flags.DEBUG_CNT_UPDATER_VERBOSE
...
# see also:
## from cnt_model.CntLadder import _rail_end_atom_to_ladder
...
class CntLadderRailChunk(Chunk):
    """
    """

    # initial values of instance variables:
    
    wholechain = None # will be a WholeChain once cnt_updater is done;
    ...
    ladder = None # will be a CntLadder in finished instances;
    ...
    def __init__(self, assy, chain, reuse_old_chunk_if_possible = False):
        ...
        # for now just assume chain is a CntChain
        ...
        if reuse_old_chunk_if_possible:
            ...
            # various ways, many of which make the cnt updater run but don't
            # require different CntLadders -- this will remake ladders and
            ...
            if old_chunk is not None:
                if _DEBUG_REUSE_CHUNKS():
                    print "cnt updater will reuse %r rather than new %r" % \
                          (old_chunk, self) 
        ...
        from cnt.model.CntLadder import _rail_end_atom_to_ladder
        ...
        if len(self._counted_chunks) == 1:
            ...
            if self._counted_atoms == len(old_chunk.atoms):
                ...
                if self.__class__ is old_chunk.__class__:
                    ...
                    # or after cnt generator, but could happen other times too.
                    if _DEBUG_REUSE_CHUNKS():
                        print "fyi: cnt updater could reuse, except for class: %r" % old_chunk
                    ...
                    # and maybe when using the Cnt Generator, so it'd be useful. I'm not sure how to detect it -- we might
    ...
    def _grab_atoms_from_chain(self, chain, just_count):
        ...
        pull in all baseatoms from the given CntChain,
        plus whatever bondpoints or Pl atoms are attached to them
        (but only Pl atoms which are not already in other CntLadderRailChunks).
        """
...
##                # usually true; if this is too slow, just do it from chunks alone
    ...
    def _set_properties_from_grab_atom_info(self): # 080201
        ...
        if self._num_old_atoms_hidden and not self._num_old_atoms_not_hidden:
            ...
            if debug_flags.DEBUG_CNT_UPDATER:
                summary_format = "CNT updater: debug fyi: remade [N] hidden chunk(s)"
                env.history.deferred_summary_message( graymsg(summary_format) )
        elif self._num_old_atoms_hidden:
            summary_format = "Warning: CNT updater unhid [N] hidden atom(s)"
            ...
            if debug_flags.DEBUG_CNT_UPDATER:
                ...
                summary_format2 = "Note: CNT updater must unhide some hidden atoms due to [N] unhidden atom(s)"
        ...
        if self._num_old_atoms_hidden + self._num_old_atoms_not_hidden > len(self.atoms):
            env.history.redmsg("Bug in CNT updater, see console prints")
            print "Bug in CNT updater: _num_old_atoms_hidden %r + self._num_old_atoms_not_hidden %r > len(self.atoms) %r, for %r" % \
    ...
    def in_a_valid_ladder(self): #bruce 071203
        """
        Is this chunk a rail of a valid CntLadder?
        [overrides Chunk method]
        """
    ...
    def addatom(self, atom):
        ...
        if self.ladder and self.ladder.valid:
            ...
            if atom.element.eltnum != 0:
                print "cnt updater, fyi: %r.addatom %r invals %r" % (self, atom, self.ladder)
            self.ladder.invalidate()
        return
    ...
    def merge(self, other): # overridden just for debug, 080120 9pm
        if debug_flags.DEBUG_CNT_UPDATER:
            print "cnt updater debug: fyi: calling %r.merge(%r)" % (self, other)
    ...
    pass # end of class CntLadderRailChunk

def _make_or_reuse_CntLadderRailChunk(constructor, assy, chain, ladder):
    """
    """
...
class CntAxisChunk(CntLadderRailChunk):
    """
    Chunk for holding part of a Cnt Segment Axis (the part in a single CntLadder).

    Internal model object; same comments as in CntStrandChunk docstring apply.
    """
...
def make_or_reuse_CntAxisChunk(assy, chain, ladder):
    """
    """
    return _make_or_reuse_CntLadderRailChunk( CntAxisChunk, assy, chain, ladder)

# ==

class CntStrandChunk_UNUSED(CntLadderRailChunk):
    """
    Chunk for holding part of a Cnt Strand (the part in a single CntLadder).

    Internal model object -- won't be directly user-visible (for MT, selection, etc)
    when cnt updater is complete. But it's a normal member of the internal model tree for
    purposes of copy, undo, mmp file, internal selection, draw.
    (Whether copy implem makes another chunk of this class, or relies on cnt
    ...
    def _grab_atoms_from_chain(self, chain, just_count): # misnamed, doesn't take them out of chain
        ...
        """
        CntLadderRailChunk._grab_atoms_from_chain(self, chain, just_count)
        for atom in chain.baseatoms:
                ...
                if grab_atom2:
                    if just_count:
                        ...
                        if atom2.molecule is self:
                            print "cnt updater: should not happen: %r is already in %r" % \
                                  (atom2, self)
    ...
    pass # end of class CntStrandChunk

def make_or_reuse_CntStrandChunk(assy, chain, ladder):
    """
    """
    return _make_or_reuse_CntLadderRailChunk(CntStrandChunk, assy, chain, ladder)
