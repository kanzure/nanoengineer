    ...
    if ignore_these or debug_print_even_if_none:
        if (not ignore_these) or changes_ok:
            if debug_flags.DEBUG_DNA_UPDATER:
                print "cnt updater: ignoring %d new changes %s" % (len(ignore_these), from_what)
        else:
            msg = "\nBUG: cnt updater: ignoring %d new changes %s -- any such changes are a bug: " % \
...
def clear_updater_run_globals(): #bruce 080218
    """
    Clear globals which are only used during individual runs of the cnt updater.
    """
