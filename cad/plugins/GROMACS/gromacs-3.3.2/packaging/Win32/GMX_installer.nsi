...
!define PRODUCT_NAME "GROMACS+HDF5"
...
Section "MCPP" SEC02
  SetOutPath "$INSTDIR\MCCP\bin"
  File ".\mcpp\bin\mcpp.exe"
  SetOutPath "$INSTDIR\MCCP"
  File ".\mcpp\LICENSE"
...
Section -AdditionalIcons
  SetOutPath $INSTDIR
  CreateDirectory "$SMPROGRAMS\GROMACS+HDF5"
  CreateShortCut "$SMPROGRAMS\GROMACS+HDF5\Uninstall.lnk" "$INSTDIR\uninst.exe"
SectionEnd
...
Section Uninstall
  ...
  Delete "$SMPROGRAMS\GROMACS+HDF5\Uninstall.lnk"

  RMDir "$SMPROGRAMS\GROMACS+HDF5"
  RMDir "$INSTDIR\MCCP\bin"
  RMDir "$INSTDIR\MCCP"
  RMDir "$INSTDIR\lib"
