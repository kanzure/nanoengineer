/*
 * $Id: gmxfio.c,v 1.36.2.2 2007/09/20 06:35:40 spoel Exp $
 * 
...
/* METHOD: do_hdf5read */
static bool do_hdf5read(void *item, int nitem, int eio, char *desc,
                        char *srcfile, int line) {
    printf(">>>gmxfio.c:do_hdf5read() not implemented\n");
    return 1;
} 
  
  
/* METHOD: do_hdf5write */
static bool do_hdf5write(void *item, int nitem, int eio, char *desc,
                         char *srcfile, int line) {
//  printf(">>>gmxfio.c:do_hdf5write() eio=%d nitem=%d desc=%s srcfile=%s line=%d\n",
//      eio, nitem, desc, srcfile, line);
    
    if (strcmp(desc, "sh->t") == 0) {
        // New frame
        addHDF5frame(*((real *)item));

    } else if (strcmp(desc, "x") == 0) {
        // Atom coordinates
        float coordinates[nitem * 3];
        int index;
        for (index = 0; index < nitem; index++) {
            real* realPtr = ((rvec*)item)[index];
            coordinates[index * 3 + 0] = realPtr[XX];
            coordinates[index * 3 + 1] = realPtr[YY];
            coordinates[index * 3 + 2] = realPtr[ZZ];
        }
        addHDF5atomCoordinates(coordinates, nitem);
    }
  ...
  case eioRVEC:
    ptr = (real *)item;
    printf("eioRVEC %18.10e%18.10e%18.10e%s\n",
          ptr[XX],ptr[YY],ptr[ZZ],dbgstr(desc));
    break;
  case eioNRVEC:
    for(i=0; (i<2); i++) { //nitem); i++) {
      ptr = ((rvec *)item)[i];
      printf("eioNRVEC %18.10e%18.10e%18.10e%s\n",
            ptr[XX],ptr[YY],ptr[ZZ],dbgstr(desc));
    }
    break;
  case eioIVEC:
    iptr= (int *)item;
    printf("%18d%18d%18d%s\n",
          iptr[XX],iptr[YY],iptr[ZZ],dbgstr(desc));
    break;
  case eioSTRING:
    encode_string(256,strbuf,(char *)item);
...
int fio_open(char *fn,char *mode)
{
  ...
  if (fn) {
    ...
    else if (fio->iFTP == efNH5) {
        openHDF5dataStore(fn);
    }
    else {
...
#endif
  else if (in_ftpset(FIO[fio].iFTP,asize(ftpNH5),ftpNH5)) {
    do_read  = do_hdf5read;
    do_write = do_hdf5write;
  }
  else 
